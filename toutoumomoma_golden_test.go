// Code generate by go test -generate github.com/kortschak/toutoumomoma. DO NOT EDIT.

// Copyright ©2021 Dan Kortschak. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package toutoumomoma

// goldenValues is the list of known golden values keyed on Go and Garble version
// at the first level and test name in the second level.
var goldenValues = map[string]map[test]interface{}{
	"go1.17.5:v0.4.0": {
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash: "f617fea3d4f7f43e80e200c8405084ae",
			imports: []string{
				"J5U4Po36.XWbBm1AJ",
				"J5U4Po36.XWbBm1AJ.func1",
				"J5U4Po36.ueJ06b5k",
				"J5U4Po36.ueJ06b5k.func1",
				"main.main",
				"main.main.func1",
			},
			entropy:  4.494293656279885,
			variance: 0.003947008480301743,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash: "4975af8e574a8a7ab7b67a5498bafb44",
			imports: []string{
				"H66IYw3R.EG258N6M",
				"main.main",
			},
			entropy:  4.132944044980959,
			variance: 0.0135547094240155,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash: "62b2f3abcd0f24edbcbd4d7e20d60d3b",
			imports: []string{
				"JwTaDs6G.Lkix37cj",
				"JwTaDs6G.f6mJljuG",
				"main.main",
			},
			entropy:  4.297675800911846,
			variance: 0.009977420320126346,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash: "c66aedc8ad4d994151824d9ae04f31d6",
			imports: []string{
				"Xhfmd6vu.ERkO_8S8",
				"main.main",
			},
			entropy:  4.132944044980958,
			variance: 0.013554709424015496,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8be36,
					Entropy:    6.167347591819188,
					VarEntropy: 9.867355524334404e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.6276890098831465,
					VarEntropy: 0.015016544046307864,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38a8f,
					Entropy:    4.376612275317795,
					VarEntropy: 5.280387075386776e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.6495169670279215,
					VarEntropy: 0.008187743833172039,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6067374825531435,
					VarEntropy: 0.034873377046304554,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x601a0,
					Entropy:    5.450014394459792,
					VarEntropy: 2.300325694260958e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.8584585933443494,
					VarEntropy: 0.05540633161466978,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599381100755206,
					VarEntropy: 0.00016907674543298053,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7609293377327442,
					VarEntropy: 0.0003625007880729216,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f088,
					Entropy:    6.1311721379240485,
					VarEntropy: 3.0386837114991553e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.565361401346228,
					VarEntropy: 0.0003931745779535265,
					Flags:      0x1,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8beb6,
					Entropy:    6.166788335680352,
					VarEntropy: 9.863806074376723e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.4893399598381714,
					VarEntropy: 0.01585074784028974,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38b2f,
					Entropy:    4.380076990934671,
					VarEntropy: 5.275236905659203e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.6495169670279215,
					VarEntropy: 0.008187743833172039,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6631709458911685,
					VarEntropy: 0.0347247167012339,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x610c0,
					Entropy:    5.47013059388382,
					VarEntropy: 2.2520357634932778e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.7959585933443494,
					VarEntropy: 0.052778240182421145,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599890017368,
					VarEntropy: 0.00016906838984816983,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7437119429494201,
					VarEntropy: 0.00036301843349099635,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f068,
					Entropy:    6.139704184531683,
					VarEntropy: 3.0284753345571777e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.653664356107006,
					VarEntropy: 0.00037981931601000385,
					Flags:      0x1,
				},
				{
					Name:       "__zdebug_abbrev",
					Size:       0x117,
					Entropy:    7.166065824433167,
					VarEntropy: 0.0022405772861620005,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_line",
					Size:       0x1d5c5,
					Entropy:    7.991127843423397,
					VarEntropy: 2.0212417105952686e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_frame",
					Size:       0x5b90,
					Entropy:    7.931221335818104,
					VarEntropy: 8.399860586863567e-06,
					Flags:      0x2000000,
				},
				{
					Name:       "__debug_gdb_scri",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_info",
					Size:       0x33b80,
					Entropy:    7.996167745278472,
					VarEntropy: 5.167859750132946e-08,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_loc",
					Size:       0x1a6ba,
					Entropy:    7.984926337700013,
					VarEntropy: 4.1118774999903916e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_ranges",
					Size:       0x838f,
					Entropy:    7.891479517426951,
					VarEntropy: 8.821148933818937e-06,
					Flags:      0x2000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.1726348037427226,
					VarEntropy: 1.0665784772777106e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35880,
					Entropy:    4.345693523795199,
					VarEntropy: 5.689945809821882e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x94,
					Entropy:    4.278922006970282,
					VarEntropy: 0.006074968357746798,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7700952245237347,
					VarEntropy: 0.008305293779624193,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.1011498751545608,
					VarEntropy: 0.046760554373954454,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x593e8,
					Entropy:    5.447476636422382,
					VarEntropy: 2.5723218153134535e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.4681390622295662,
					VarEntropy: 0.08313705052107564,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.6079008723320936,
					VarEntropy: 0.0001693604060627171,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x77f0,
					Entropy:    1.582695899677633,
					VarEntropy: 0.0003591461339251088,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.1720093737546,
					VarEntropy: 1.0671493890547944e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35920,
					Entropy:    4.351078638675726,
					VarEntropy: 5.6869549960717754e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x17a,
					Entropy:    4.332514286812163,
					VarEntropy: 0.0018720932548837838,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7700952245237347,
					VarEntropy: 0.008305293779624193,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.045054107939454,
					VarEntropy: 0.04696294988334262,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x5a1e8,
					Entropy:    5.474218747550726,
					VarEntropy: 2.499111219906954e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.5608203810934285,
					VarEntropy: 0.07164523745026193,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.60821346272867,
					VarEntropy: 0.0001693709947955346,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x7810,
					Entropy:    1.6046467490248555,
					VarEntropy: 0.0003631319373646502,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    7.9939814167373955,
					VarEntropy: 8.816451131074686e-08,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    7.97808162806558,
					VarEntropy: 2.9092720639125194e-06,
					Flags:      0x3,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x119,
					Entropy:    7.186678878967755,
					VarEntropy: 0.0023487100483950583,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1b8ac,
					Entropy:    7.991399871405648,
					VarEntropy: 2.1037107874221076e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5526,
					Entropy:    7.922440004034238,
					VarEntropy: 1.016144764855772e-05,
					Flags:      0x0,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x31a5e,
					Entropy:    7.995541426118696,
					VarEntropy: 6.267550488793614e-08,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x198ca,
					Entropy:    7.98781739055232,
					VarEntropy: 3.42170689161735e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x8fac,
					Entropy:    7.784901524718818,
					VarEntropy: 1.6544706982903474e-05,
					Flags:      0x0,
				},
				{
					Name:       ".note.go.buildid",
					Size:       0x64,
					Entropy:    5.267368784857279,
					VarEntropy: 0.011785373992272965,
					Flags:      0x2,
				},
				{
					Name:       ".symtab",
					Size:       0xc5e8,
					Entropy:    3.20652780203368,
					VarEntropy: 0.0002942263172418581,
					Flags:      0x0,
				},
				{
					Name:       ".strtab",
					Size:       0xb288,
					Entropy:    4.811184615055419,
					VarEntropy: 4.984016326397377e-05,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x10fa88,
					Entropy:    5.877353366653366,
					VarEntropy: 7.179499001647305e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640950842399639,
					VarEntropy: 0.00015375648698920822,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x1108a8,
					Entropy:    5.881198482715571,
					VarEntropy: 7.124647426082509e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.641107213427654,
					VarEntropy: 0.00015378291680262545,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0xe9ac,
					Entropy:    5.09491297332817,
					VarEntropy: 8.573480749784487e-05,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.174219578291682,
					VarEntropy: 9.747369960039817e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9d600,
					Entropy:    5.131207218044359,
					VarEntropy: 1.671073994258933e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.605289268139611,
					VarEntropy: 0.0001513937129465353,
					Flags:      0xc0000040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.5545243710280534,
					VarEntropy: 0.0051654859306460325,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.4381901305203595,
					VarEntropy: 2.3255531494436373e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x200,
					Entropy:    0.020393135236084953,
					VarEntropy: 0.0003081937203618641,
					Flags:      0x42000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.1761252724928655,
					VarEntropy: 9.747108821196161e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9e600,
					Entropy:    5.138494981029609,
					VarEntropy: 1.6482890827748315e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.601169638514693,
					VarEntropy: 0.0001514039073443931,
					Flags:      0xc0000040,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x200,
					Entropy:    4.829215920067953,
					VarEntropy: 0.024587620899965145,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1cc00,
					Entropy:    7.992417306885133,
					VarEntropy: 1.7899626019515757e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5800,
					Entropy:    7.9257471578709,
					VarEntropy: 9.304797839881427e-06,
					Flags:      0x42100040,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x200,
					Entropy:    0.7691902074556789,
					VarEntropy: 0.008919886275183865,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x32a00,
					Entropy:    7.996420128772403,
					VarEntropy: 4.993294443511977e-08,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x1ba00,
					Entropy:    7.988932433866987,
					VarEntropy: 2.8483765422067984e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x9600,
					Entropy:    7.777719231416577,
					VarEntropy: 1.6851116730811515e-05,
					Flags:      0x42100040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.6148445724061844,
					VarEntropy: 0.005141851830632267,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.440793703442042,
					VarEntropy: 2.360335708126473e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x17800,
					Entropy:    5.133671236865931,
					VarEntropy: 6.743980706348455e-05,
					Flags:      0x42000000,
				},
			},
		},
	},
	"go1.17.5:v0.5.0": {
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash: "72767423e8c669a069693b2144dc924f",
			imports: []string{
				"P1GpSVIn.Cx_z5Kiv",
				"P1GpSVIn.Cx_z5Kiv.func1",
				"P1GpSVIn.Cx_z5Kiv.func1.1",
				"P1GpSVIn.bis_10t4",
				"P1GpSVIn.bis_10t4.func1",
				"main.main",
				"main.main.func1",
			},
			entropy:  4.511452451546035,
			variance: 0.005691093034499717,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash: "1827c106bb4b85708c25e552663f15ba",
			imports: []string{
				"jcBkD03T.ZzzTfZmq",
				"main.main",
			},
			entropy:  3.979097891134805,
			variance: 0.011845611789894671,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash: "8b4eb0e4f4d2ac3d4b1e6282eb3662ad",
			imports: []string{
				"main.main",
				"vaispnVn.JKd9hry7",
				"vaispnVn.xAqgDwtm",
			},
			entropy:  4.286275219617123,
			variance: 0.021245564915371048,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash: "0236bbd99b42899645333a3c789057ce",
			imports: []string{
				"DRwvqhLK.RxUijwao",
				"main.main",
			},
			entropy:  3.9500637564362093,
			variance: 0.015433388447686188,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8be36,
					Entropy:    6.167347591819188,
					VarEntropy: 9.867355524334404e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.6276890098831465,
					VarEntropy: 0.015016544046307864,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38a8f,
					Entropy:    4.376612275317795,
					VarEntropy: 5.280387075386776e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.6495169670279215,
					VarEntropy: 0.008187743833172039,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6067374825531435,
					VarEntropy: 0.034873377046304554,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x601a0,
					Entropy:    5.450024725788533,
					VarEntropy: 2.3003525658832268e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.8584585933443494,
					VarEntropy: 0.05540633161466978,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599381100755206,
					VarEntropy: 0.00016907674543298053,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7609293377327442,
					VarEntropy: 0.0003625007880729216,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f088,
					Entropy:    6.1311721379240485,
					VarEntropy: 3.0386837114991553e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.565361401346228,
					VarEntropy: 0.0003931745779535265,
					Flags:      0x1,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8beb6,
					Entropy:    6.166788335680352,
					VarEntropy: 9.863806074376723e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.4893399598381714,
					VarEntropy: 0.01585074784028974,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38b2f,
					Entropy:    4.380076990934671,
					VarEntropy: 5.275236905659203e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.6495169670279215,
					VarEntropy: 0.008187743833172039,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6631709458911685,
					VarEntropy: 0.0347247167012339,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x610c0,
					Entropy:    5.47013059388382,
					VarEntropy: 2.2520357634932778e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.7959585933443494,
					VarEntropy: 0.052778240182421145,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599890017368,
					VarEntropy: 0.00016906838984816983,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7437119429494201,
					VarEntropy: 0.00036301843349099635,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f068,
					Entropy:    6.139704184531683,
					VarEntropy: 3.0284753345571777e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.653664356107006,
					VarEntropy: 0.00037981931601000385,
					Flags:      0x1,
				},
				{
					Name:       "__zdebug_abbrev",
					Size:       0x117,
					Entropy:    7.166065824433167,
					VarEntropy: 0.0022405772861620005,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_line",
					Size:       0x1d5c5,
					Entropy:    7.991127843423397,
					VarEntropy: 2.0212417105952686e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_frame",
					Size:       0x5b90,
					Entropy:    7.931221335818104,
					VarEntropy: 8.399860586863567e-06,
					Flags:      0x2000000,
				},
				{
					Name:       "__debug_gdb_scri",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_info",
					Size:       0x33b80,
					Entropy:    7.996167745278472,
					VarEntropy: 5.167859750132946e-08,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_loc",
					Size:       0x1a6ba,
					Entropy:    7.984926337700013,
					VarEntropy: 4.1118774999903916e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_ranges",
					Size:       0x838f,
					Entropy:    7.891479517426951,
					VarEntropy: 8.821148933818937e-06,
					Flags:      0x2000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.172594619836122,
					VarEntropy: 1.0665662649031311e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35880,
					Entropy:    4.345693523795199,
					VarEntropy: 5.689945809821882e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x94,
					Entropy:    4.278922006970282,
					VarEntropy: 0.006074968357746798,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7700952245237347,
					VarEntropy: 0.008305293779624193,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.164085749945878,
					VarEntropy: 0.04975413105388619,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x593e8,
					Entropy:    5.447480865103664,
					VarEntropy: 2.572318963458046e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.4681390622295662,
					VarEntropy: 0.08313705052107564,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.607894681959076,
					VarEntropy: 0.00016936120118816805,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x77f0,
					Entropy:    1.583222352311009,
					VarEntropy: 0.00035928131568237166,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.1720093737546,
					VarEntropy: 1.0671493890547944e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35920,
					Entropy:    4.351078638675726,
					VarEntropy: 5.6869549960717754e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x17a,
					Entropy:    4.332514286812163,
					VarEntropy: 0.0018720932548837838,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7700952245237347,
					VarEntropy: 0.008305293779624193,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.045054107939454,
					VarEntropy: 0.04696294988334262,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x5a1e8,
					Entropy:    5.474218747550726,
					VarEntropy: 2.499111219906954e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.5608203810934285,
					VarEntropy: 0.07164523745026193,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.60821346272867,
					VarEntropy: 0.0001693709947955346,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x7810,
					Entropy:    1.6046467490248555,
					VarEntropy: 0.0003631319373646502,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    7.9939814167373955,
					VarEntropy: 8.816451131074686e-08,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    7.97808162806558,
					VarEntropy: 2.9092720639125194e-06,
					Flags:      0x3,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x119,
					Entropy:    7.186678878967755,
					VarEntropy: 0.0023487100483950583,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1b8ac,
					Entropy:    7.991399871405648,
					VarEntropy: 2.1037107874221076e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5526,
					Entropy:    7.922440004034238,
					VarEntropy: 1.016144764855772e-05,
					Flags:      0x0,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x31a5e,
					Entropy:    7.995541426118696,
					VarEntropy: 6.267550488793614e-08,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x198ca,
					Entropy:    7.98781739055232,
					VarEntropy: 3.42170689161735e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x8fac,
					Entropy:    7.784901524718818,
					VarEntropy: 1.6544706982903474e-05,
					Flags:      0x0,
				},
				{
					Name:       ".note.go.buildid",
					Size:       0x64,
					Entropy:    5.267368784857279,
					VarEntropy: 0.011785373992272965,
					Flags:      0x2,
				},
				{
					Name:       ".symtab",
					Size:       0xc5e8,
					Entropy:    3.20652780203368,
					VarEntropy: 0.0002942263172418581,
					Flags:      0x0,
				},
				{
					Name:       ".strtab",
					Size:       0xb288,
					Entropy:    4.811184615055419,
					VarEntropy: 4.984016326397377e-05,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x10fa88,
					Entropy:    5.877361482116572,
					VarEntropy: 7.179522220502871e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640912561947339,
					VarEntropy: 0.0001537561659576765,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x1108a8,
					Entropy:    5.881198482715571,
					VarEntropy: 7.124647426082509e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.641107213427654,
					VarEntropy: 0.00015378291680262545,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0xe9ac,
					Entropy:    5.09491297332817,
					VarEntropy: 8.573480749784487e-05,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.174182447691865,
					VarEntropy: 9.747268072924351e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9d600,
					Entropy:    5.131147594027367,
					VarEntropy: 1.6710080254076667e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.605350046878938,
					VarEntropy: 0.00015139096991241227,
					Flags:      0xc0000040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.5545243710280534,
					VarEntropy: 0.0051654859306460325,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.438189379063121,
					VarEntropy: 2.325563575996334e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x200,
					Entropy:    0.020393135236084953,
					VarEntropy: 0.0003081937203618641,
					Flags:      0x42000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.1761252724928655,
					VarEntropy: 9.747108821196161e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9e600,
					Entropy:    5.138494981029609,
					VarEntropy: 1.6482890827748315e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.601169638514693,
					VarEntropy: 0.0001514039073443931,
					Flags:      0xc0000040,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x200,
					Entropy:    4.829215920067953,
					VarEntropy: 0.024587620899965145,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1cc00,
					Entropy:    7.992417306885133,
					VarEntropy: 1.7899626019515757e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5800,
					Entropy:    7.9257471578709,
					VarEntropy: 9.304797839881427e-06,
					Flags:      0x42100040,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x200,
					Entropy:    0.7691902074556789,
					VarEntropy: 0.008919886275183865,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x32a00,
					Entropy:    7.996420128772403,
					VarEntropy: 4.993294443511977e-08,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x1ba00,
					Entropy:    7.988932433866987,
					VarEntropy: 2.8483765422067984e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x9600,
					Entropy:    7.777719231416577,
					VarEntropy: 1.6851116730811515e-05,
					Flags:      0x42100040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.6148445724061844,
					VarEntropy: 0.005141851830632267,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.440793703442042,
					VarEntropy: 2.360335708126473e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x17800,
					Entropy:    5.133671236865931,
					VarEntropy: 6.743980706348455e-05,
					Flags:      0x42000000,
				},
			},
		},
	},
	"go1.17.6:v0.4.0": {
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash: "c178eff7cd93ad039d4f2643c67aa967",
			imports: []string{
				"BUxXhmgz.CyxbaAxE",
				"BUxXhmgz.CyxbaAxE.func1",
				"BUxXhmgz.biKdRbFl",
				"BUxXhmgz.biKdRbFl.func1",
				"main.main",
				"main.main.func1",
			},
			entropy:  4.510731213438988,
			variance: 0.005118779514582775,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash: "018fb06a2cd14862ee92e8344afee4a1",
			imports: []string{
				"main.main",
				"o5P4rrtF.REIQ_I2f",
			},
			entropy:  4.1619781796795525,
			variance: 0.009558488848429664,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash: "7784915dfc22f2f293a223e4b02c9de1",
			imports: []string{
				"main.main",
				"ybHCerLx.VDhGo61W",
				"ybHCerLx.cw80TErc",
			},
			entropy:  4.646967661578217,
			variance: 0.0070084188859092875,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash: "a9327f6811bbc246bdd318ca4c199df2",
			imports: []string{
				"VmSVL5rW.SyaODDHs",
				"main.main",
			},
			entropy:  3.873140679513133,
			variance: 0.013724290813565352,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8be36,
					Entropy:    6.167151561129461,
					VarEntropy: 9.866087709269723e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.6276890098831465,
					VarEntropy: 0.015016544046307864,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38aaf,
					Entropy:    4.374379797891957,
					VarEntropy: 5.268887713049234e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.713889346674307,
					VarEntropy: 0.00805212555333465,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6631709458911685,
					VarEntropy: 0.0347247167012339,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x601a0,
					Entropy:    5.448555797699439,
					VarEntropy: 2.2992099696043863e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.8584585933443494,
					VarEntropy: 0.05540633161466978,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599370840155541,
					VarEntropy: 0.00016907716010970933,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.760965834254384,
					VarEntropy: 0.0003621253770549902,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f088,
					Entropy:    6.1313687108723,
					VarEntropy: 3.0383223971639575e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.565359757429149,
					VarEntropy: 0.00039310843844961475,
					Flags:      0x1,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8beb6,
					Entropy:    6.166834790676923,
					VarEntropy: 9.863339211204593e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.4893399598381714,
					VarEntropy: 0.01585074784028974,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38b4f,
					Entropy:    4.378870933535319,
					VarEntropy: 5.2726507723453276e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.713889346674307,
					VarEntropy: 0.00805212555333465,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.662338733640253,
					VarEntropy: 0.03458699992230293,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x610c0,
					Entropy:    5.469096410789927,
					VarEntropy: 2.2512068120373644e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.7959585933443494,
					VarEntropy: 0.052778240182421145,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599716173505525,
					VarEntropy: 0.00016908038331659258,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7437891316045526,
					VarEntropy: 0.0003620706040999194,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f068,
					Entropy:    6.140320504766198,
					VarEntropy: 3.0283389606051662e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.655245963076966,
					VarEntropy: 0.00037966907625685826,
					Flags:      0x1,
				},
				{
					Name:       "__zdebug_abbrev",
					Size:       0x117,
					Entropy:    7.166065824433167,
					VarEntropy: 0.0022405772861620005,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_line",
					Size:       0x1d5d0,
					Entropy:    7.990324720568736,
					VarEntropy: 2.1962513113172782e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_frame",
					Size:       0x5b90,
					Entropy:    7.931221335818104,
					VarEntropy: 8.399860586863567e-06,
					Flags:      0x2000000,
				},
				{
					Name:       "__debug_gdb_scri",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_info",
					Size:       0x33b9a,
					Entropy:    7.9961831548329005,
					VarEntropy: 5.116688887026936e-08,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_loc",
					Size:       0x1a6ba,
					Entropy:    7.984926337700013,
					VarEntropy: 4.1118774999903916e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_ranges",
					Size:       0x838f,
					Entropy:    7.891479517426951,
					VarEntropy: 8.821148933818937e-06,
					Flags:      0x2000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.1726267906855545,
					VarEntropy: 1.0666500878507862e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x358a0,
					Entropy:    4.348062560038336,
					VarEntropy: 5.6842742397058924e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x94,
					Entropy:    4.278922006970282,
					VarEntropy: 0.006074968357746798,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7941638566785665,
					VarEntropy: 0.008160113381189311,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.135389005131675,
					VarEntropy: 0.04770326580473294,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x593c8,
					Entropy:    5.450203853467008,
					VarEntropy: 2.5531310165054004e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.4681390622295662,
					VarEntropy: 0.08313705052107564,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.608059700687032,
					VarEntropy: 0.00016935717275402414,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x77f0,
					Entropy:    1.5859465586567814,
					VarEntropy: 0.00036012361195304527,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.171960423156391,
					VarEntropy: 1.0670534574051352e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35940,
					Entropy:    4.355543356513369,
					VarEntropy: 5.691361796439461e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x17a,
					Entropy:    4.332514286812163,
					VarEntropy: 0.0018720932548837838,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7941638566785665,
					VarEntropy: 0.008160113381189311,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.149135857994784,
					VarEntropy: 0.046165397750464555,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x5a1e8,
					Entropy:    5.477551319776382,
					VarEntropy: 2.5029844485770247e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.5608203810934285,
					VarEntropy: 0.07164523745026193,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.608490555242617,
					VarEntropy: 0.00016935185165468147,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x7810,
					Entropy:    1.6063005035044693,
					VarEntropy: 0.0003638549350251033,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    7.993708093489265,
					VarEntropy: 9.153220121303988e-08,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    7.975666586127946,
					VarEntropy: 3.2269952561131864e-06,
					Flags:      0x3,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x119,
					Entropy:    7.186678878967755,
					VarEntropy: 0.0023487100483950583,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1b8a4,
					Entropy:    7.991902291817906,
					VarEntropy: 1.9590073032651564e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5526,
					Entropy:    7.922440004034238,
					VarEntropy: 1.016144764855772e-05,
					Flags:      0x0,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x31a82,
					Entropy:    7.99573472077519,
					VarEntropy: 6.04950156663721e-08,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x198ca,
					Entropy:    7.98781739055232,
					VarEntropy: 3.42170689161735e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x8fac,
					Entropy:    7.784901524718818,
					VarEntropy: 1.6544706982903474e-05,
					Flags:      0x0,
				},
				{
					Name:       ".note.go.buildid",
					Size:       0x64,
					Entropy:    5.320818564536743,
					VarEntropy: 0.01119348465019243,
					Flags:      0x2,
				},
				{
					Name:       ".symtab",
					Size:       0xc5e8,
					Entropy:    3.2070627267031826,
					VarEntropy: 0.0002941954027828894,
					Flags:      0x0,
				},
				{
					Name:       ".strtab",
					Size:       0xb288,
					Entropy:    4.811184615055419,
					VarEntropy: 4.984016326397377e-05,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x10fac8,
					Entropy:    5.8674574438357805,
					VarEntropy: 7.175439783426715e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640588822373945,
					VarEntropy: 0.0001537574418463962,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x1108e8,
					Entropy:    5.877301101198294,
					VarEntropy: 7.123723613664263e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640976543009014,
					VarEntropy: 0.00015377671681234716,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0xe9ac,
					Entropy:    5.094666828676649,
					VarEntropy: 8.570249690578983e-05,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.174253883550948,
					VarEntropy: 9.747273424052483e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9d600,
					Entropy:    5.131546236507263,
					VarEntropy: 1.6715457161442527e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.60490449676488,
					VarEntropy: 0.0001514073525734191,
					Flags:      0xc0000040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.5545243710280534,
					VarEntropy: 0.0051654859306460325,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.4380137125747074,
					VarEntropy: 2.410565564699714e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x200,
					Entropy:    0.020393135236084953,
					VarEntropy: 0.0003081937203618641,
					Flags:      0x42000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.176356053796499,
					VarEntropy: 9.748148839221063e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9e600,
					Entropy:    5.1409384253329655,
					VarEntropy: 1.6485943529170202e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.601328355315124,
					VarEntropy: 0.0001514044694401641,
					Flags:      0xc0000040,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x200,
					Entropy:    4.829215920067953,
					VarEntropy: 0.024587620899965145,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1cc00,
					Entropy:    7.993024884883035,
					VarEntropy: 1.6525580749442324e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5800,
					Entropy:    7.9257471578709,
					VarEntropy: 9.304797839881427e-06,
					Flags:      0x42100040,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x200,
					Entropy:    0.7691902074556789,
					VarEntropy: 0.008919886275183865,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x32a00,
					Entropy:    7.996494978802828,
					VarEntropy: 4.884646450815275e-08,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x1ba00,
					Entropy:    7.988932433866987,
					VarEntropy: 2.8483765422067984e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x9600,
					Entropy:    7.777719231416577,
					VarEntropy: 1.6851116730811515e-05,
					Flags:      0x42100040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.6148445724061844,
					VarEntropy: 0.005141851830632267,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.438815992577784,
					VarEntropy: 2.3712057587464778e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x17800,
					Entropy:    5.133872045459199,
					VarEntropy: 6.74350759437562e-05,
					Flags:      0x42000000,
				},
			},
		},
	},
	"go1.17.6:v0.5.0": {
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash: "1af711fc66a5e81af28bbe0d73db85d9",
			imports: []string{
				"main.main",
				"main.main.func1",
				"u0zrkBqk.QlKevWvi",
				"u0zrkBqk.QlKevWvi.func1",
				"u0zrkBqk.iBk7lETX",
				"u0zrkBqk.iBk7lETX.func1",
			},
			entropy:  4.4424972913961085,
			variance: 0.005510613184736628,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash: "609cec24551f28a9bb3db784cbb8133c",
			imports: []string{
				"main.main",
				"pC2mY22B.SK7bTzZA",
			},
			entropy:  4.026986833359287,
			variance: 0.016687319946167515,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash: "1dc3c74988728cc38289a60012d8fffe",
			imports: []string{
				"JeRteOwo.XeWrGBIl",
				"JeRteOwo.kL7Ks7ae",
				"main.main",
			},
			entropy:  4.379298475431078,
			variance: 0.015160017731717152,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash: "72c12746df84f994a762f2d54810a7ae",
			imports: []string{
				"eBOnUb76.HajuTrE2",
				"main.main",
			},
			entropy:  4.103909910282364,
			variance: 0.017486085309009327,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8be36,
					Entropy:    6.167151561129461,
					VarEntropy: 9.866087709269723e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.6276890098831465,
					VarEntropy: 0.015016544046307864,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38aaf,
					Entropy:    4.374379797891957,
					VarEntropy: 5.268887713049234e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.713889346674307,
					VarEntropy: 0.00805212555333465,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6631709458911685,
					VarEntropy: 0.0347247167012339,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x601a0,
					Entropy:    5.448540433705996,
					VarEntropy: 2.299216942779832e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.8584585933443494,
					VarEntropy: 0.05540633161466978,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599370840155541,
					VarEntropy: 0.00016907716010970933,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.760965834254384,
					VarEntropy: 0.0003621253770549902,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f088,
					Entropy:    6.1313687108723,
					VarEntropy: 3.0383223971639575e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.565359757429149,
					VarEntropy: 0.00039310843844961475,
					Flags:      0x1,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8beb6,
					Entropy:    6.166834790676923,
					VarEntropy: 9.863339211204593e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.4893399598381714,
					VarEntropy: 0.01585074784028974,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38b4f,
					Entropy:    4.378870933535319,
					VarEntropy: 5.2726507723453276e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.713889346674307,
					VarEntropy: 0.00805212555333465,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.662338733640253,
					VarEntropy: 0.03458699992230293,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x610c0,
					Entropy:    5.469096410789927,
					VarEntropy: 2.2512068120373644e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.7959585933443494,
					VarEntropy: 0.052778240182421145,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599716173505525,
					VarEntropy: 0.00016908038331659258,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7437891316045526,
					VarEntropy: 0.0003620706040999194,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f068,
					Entropy:    6.140320504766198,
					VarEntropy: 3.0283389606051662e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.655245963076966,
					VarEntropy: 0.00037966907625685826,
					Flags:      0x1,
				},
				{
					Name:       "__zdebug_abbrev",
					Size:       0x117,
					Entropy:    7.166065824433167,
					VarEntropy: 0.0022405772861620005,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_line",
					Size:       0x1d5d0,
					Entropy:    7.990324720568736,
					VarEntropy: 2.1962513113172782e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_frame",
					Size:       0x5b90,
					Entropy:    7.931221335818104,
					VarEntropy: 8.399860586863567e-06,
					Flags:      0x2000000,
				},
				{
					Name:       "__debug_gdb_scri",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_info",
					Size:       0x33b9a,
					Entropy:    7.9961831548329005,
					VarEntropy: 5.116688887026936e-08,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_loc",
					Size:       0x1a6ba,
					Entropy:    7.984926337700013,
					VarEntropy: 4.1118774999903916e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_ranges",
					Size:       0x838f,
					Entropy:    7.891479517426951,
					VarEntropy: 8.821148933818937e-06,
					Flags:      0x2000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.1726267906855545,
					VarEntropy: 1.0666500878507862e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x358a0,
					Entropy:    4.348062560038336,
					VarEntropy: 5.6842742397058924e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x94,
					Entropy:    4.278922006970282,
					VarEntropy: 0.006074968357746798,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7941638566785665,
					VarEntropy: 0.008160113381189311,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.135389005131675,
					VarEntropy: 0.04770326580473294,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x593c8,
					Entropy:    5.4503545989450695,
					VarEntropy: 2.5532807852038393e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.4681390622295662,
					VarEntropy: 0.08313705052107564,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.608059700687032,
					VarEntropy: 0.00016935717275402414,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x77f0,
					Entropy:    1.5859465586567814,
					VarEntropy: 0.00036012361195304527,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.171960423156391,
					VarEntropy: 1.0670534574051352e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35940,
					Entropy:    4.355543356513369,
					VarEntropy: 5.691361796439461e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x17a,
					Entropy:    4.332514286812163,
					VarEntropy: 0.0018720932548837838,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7941638566785665,
					VarEntropy: 0.008160113381189311,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.149135857994784,
					VarEntropy: 0.046165397750464555,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x5a1e8,
					Entropy:    5.477551319776382,
					VarEntropy: 2.5029844485770247e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.5608203810934285,
					VarEntropy: 0.07164523745026193,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.608490555242617,
					VarEntropy: 0.00016935185165468147,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x7810,
					Entropy:    1.6063005035044693,
					VarEntropy: 0.0003638549350251033,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    7.993708093489265,
					VarEntropy: 9.153220121303988e-08,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    7.975666586127946,
					VarEntropy: 3.2269952561131864e-06,
					Flags:      0x3,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x119,
					Entropy:    7.186678878967755,
					VarEntropy: 0.0023487100483950583,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1b8a4,
					Entropy:    7.991902291817906,
					VarEntropy: 1.9590073032651564e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5526,
					Entropy:    7.922440004034238,
					VarEntropy: 1.016144764855772e-05,
					Flags:      0x0,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x31a82,
					Entropy:    7.99573472077519,
					VarEntropy: 6.04950156663721e-08,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x198ca,
					Entropy:    7.98781739055232,
					VarEntropy: 3.42170689161735e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x8fac,
					Entropy:    7.784901524718818,
					VarEntropy: 1.6544706982903474e-05,
					Flags:      0x0,
				},
				{
					Name:       ".note.go.buildid",
					Size:       0x64,
					Entropy:    5.320818564536743,
					VarEntropy: 0.01119348465019243,
					Flags:      0x2,
				},
				{
					Name:       ".symtab",
					Size:       0xc5e8,
					Entropy:    3.2070627267031826,
					VarEntropy: 0.0002941954027828894,
					Flags:      0x0,
				},
				{
					Name:       ".strtab",
					Size:       0xb288,
					Entropy:    4.811184615055419,
					VarEntropy: 4.984016326397377e-05,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x10fac8,
					Entropy:    5.867407583534304,
					VarEntropy: 7.17542567614692e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.64053401253701,
					VarEntropy: 0.0001537561452524848,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x1108e8,
					Entropy:    5.877301101198294,
					VarEntropy: 7.123723613664263e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640976543009014,
					VarEntropy: 0.00015377671681234716,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0xe9ac,
					Entropy:    5.094666828676649,
					VarEntropy: 8.570249690578983e-05,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.174256949782596,
					VarEntropy: 9.74734463281296e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9d600,
					Entropy:    5.131498537926754,
					VarEntropy: 1.6715008471032244e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.605519103098159,
					VarEntropy: 0.00015139746522973072,
					Flags:      0xc0000040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.5545243710280534,
					VarEntropy: 0.0051654859306460325,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.437602687279737,
					VarEntropy: 2.3922853759211727e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x200,
					Entropy:    0.020393135236084953,
					VarEntropy: 0.0003081937203618641,
					Flags:      0x42000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.176356053796499,
					VarEntropy: 9.748148839221063e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9e600,
					Entropy:    5.1409384253329655,
					VarEntropy: 1.6485943529170202e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.601328355315124,
					VarEntropy: 0.0001514044694401641,
					Flags:      0xc0000040,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x200,
					Entropy:    4.829215920067953,
					VarEntropy: 0.024587620899965145,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1cc00,
					Entropy:    7.993024884883035,
					VarEntropy: 1.6525580749442324e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5800,
					Entropy:    7.9257471578709,
					VarEntropy: 9.304797839881427e-06,
					Flags:      0x42100040,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x200,
					Entropy:    0.7691902074556789,
					VarEntropy: 0.008919886275183865,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x32a00,
					Entropy:    7.996494978802828,
					VarEntropy: 4.884646450815275e-08,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x1ba00,
					Entropy:    7.988932433866987,
					VarEntropy: 2.8483765422067984e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x9600,
					Entropy:    7.777719231416577,
					VarEntropy: 1.6851116730811515e-05,
					Flags:      0x42100040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.6148445724061844,
					VarEntropy: 0.005141851830632267,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.438815992577784,
					VarEntropy: 2.3712057587464778e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x17800,
					Entropy:    5.133872045459199,
					VarEntropy: 6.74350759437562e-05,
					Flags:      0x42000000,
				},
			},
		},
	},
	"go1.17.7:v0.5.0": {
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash: "1fe45fd45b3e60faf6ecb0d14aba9178",
			imports: []string{
				"main.main",
				"main.main.func1",
				"nmZ1Azmf.JH0Tfz3O",
				"nmZ1Azmf.JH0Tfz3O.func1",
				"nmZ1Azmf.rR10zzFM",
				"nmZ1Azmf.rR10zzFM.func1",
			},
			entropy:  4.1293788642391,
			variance: 0.008609760187275285,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash: "566c795f608d7d2af0f2ec1906118cbd",
			imports: []string{
				"AwxkD8k3.AIvuzuv2",
				"main.main",
			},
			entropy:  4.008132025833399,
			variance: 0.008192990441511139,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash: "3c54fdc9febdde1fe4ca8563a70f66ac",
			imports: []string{
				"main.main",
				"xYI3GdnT.TSjA7v4D",
				"xYI3GdnT.zYLzL647",
			},
			entropy:  4.350342370830228,
			variance: 0.006939842717804039,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash: "c83cbcaf5bfa37a5eb3fecd60a9f3e24",
			imports: []string{
				"main.main",
				"uiKAccnq.UZExfkWO",
			},
			entropy:  4.026986833359287,
			variance: 0.01668731994616751,
		},
		{
			name:    "GoSymbolHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: symbolHashTestResults{
			hash:     "d41d8cd98f00b204e9800998ecf8427e",
			imports:  []string(nil),
			entropy:  0.0,
			variance: 0.0,
		},
		{
			name:    "GoSymbolHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: symbolHashTestResults{
			hash: "b10a099a8babcdf0283916af8fa87240",
			imports: []string{
				"github.com/kortschak/toutoumomoma/testdata/b.Used",
				"github.com/kortschak/toutoumomoma/testdata/b.hash",
			},
			entropy:  3.8913328650565453,
			variance: 0.0072997875567589006,
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "d3ccf195b62a9279c3c19af1080497ec",
			imports: []string{
				"___error",
				"__exit",
				"_clock_gettime",
				"_close",
				"_closedir",
				"_execve",
				"_fcntl",
				"_fstat64",
				"_getcwd",
				"_getpid",
				"_kevent",
				"_kill",
				"_kqueue",
				"_lseek",
				"_mach_absolute_time",
				"_mach_timebase_info",
				"_madvise",
				"_mmap",
				"_munmap",
				"_open",
				"_pipe",
				"_pthread_attr_getstacksize",
				"_pthread_attr_init",
				"_pthread_attr_setdetachstate",
				"_pthread_cond_init",
				"_pthread_cond_signal",
				"_pthread_cond_timedwait_relative_np",
				"_pthread_cond_wait",
				"_pthread_create",
				"_pthread_kill",
				"_pthread_mutex_init",
				"_pthread_mutex_lock",
				"_pthread_mutex_unlock",
				"_pthread_self",
				"_pthread_sigmask",
				"_raise",
				"_read",
				"_sigaction",
				"_sigaltstack",
				"_stat64",
				"_sysctl",
				"_usleep",
				"_write",
			},
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash:    "d41d8cd98f00b204e9800998ecf8427e",
			imports: []string(nil),
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals -tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-literals",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "-tiny",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "ImportHash",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: importHashTestResults{
			hash: "c7269d59926fa4252270f407e4dab043",
			imports: []string{
				"kernel32.writefile",
				"kernel32.writeconsolew",
				"kernel32.waitformultipleobjects",
				"kernel32.waitforsingleobject",
				"kernel32.virtualquery",
				"kernel32.virtualfree",
				"kernel32.virtualalloc",
				"kernel32.switchtothread",
				"kernel32.suspendthread",
				"kernel32.sleep",
				"kernel32.setwaitabletimer",
				"kernel32.setunhandledexceptionfilter",
				"kernel32.setprocesspriorityboost",
				"kernel32.setevent",
				"kernel32.seterrormode",
				"kernel32.setconsolectrlhandler",
				"kernel32.resumethread",
				"kernel32.postqueuedcompletionstatus",
				"kernel32.loadlibrarya",
				"kernel32.loadlibraryw",
				"kernel32.setthreadcontext",
				"kernel32.getthreadcontext",
				"kernel32.getsysteminfo",
				"kernel32.getsystemdirectorya",
				"kernel32.getstdhandle",
				"kernel32.getqueuedcompletionstatusex",
				"kernel32.getprocessaffinitymask",
				"kernel32.getprocaddress",
				"kernel32.getenvironmentstringsw",
				"kernel32.getconsolemode",
				"kernel32.freeenvironmentstringsw",
				"kernel32.exitprocess",
				"kernel32.duplicatehandle",
				"kernel32.createwaitabletimerexw",
				"kernel32.createthread",
				"kernel32.createiocompletionport",
				"kernel32.createfilea",
				"kernel32.createeventa",
				"kernel32.closehandle",
				"kernel32.addvectoredexceptionhandler",
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8be36,
					Entropy:    6.167170864211861,
					VarEntropy: 9.866163737715117e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.6276890098831465,
					VarEntropy: 0.015016544046307864,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38aaf,
					Entropy:    4.374379797891957,
					VarEntropy: 5.268887713049234e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.713889346674307,
					VarEntropy: 0.00805212555333465,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.6453233671405316,
					VarEntropy: 0.03436781750299644,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x601a0,
					Entropy:    5.448523811184475,
					VarEntropy: 2.299180401957211e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.8584585933443494,
					VarEntropy: 0.05540633161466978,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.5993794466721,
					VarEntropy: 0.0001690781689171491,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7611624613127252,
					VarEntropy: 0.0003623014064331927,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f088,
					Entropy:    6.1313967717168305,
					VarEntropy: 3.0383178946964848e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.565359757429149,
					VarEntropy: 0.00039310843844961475,
					Flags:      0x1,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "darwin",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "__text",
					Size:       0x8beb6,
					Entropy:    6.166810286996415,
					VarEntropy: 9.863454068098746e-06,
					Flags:      0x80000400,
				},
				{
					Name:       "__symbol_stub1",
					Size:       0x102,
					Entropy:    3.4893399598381714,
					VarEntropy: 0.01585074784028974,
					Flags:      0x80000408,
				},
				{
					Name:       "__rodata",
					Size:       0x38b4f,
					Entropy:    4.378876525799632,
					VarEntropy: 5.272667187574317e-05,
					Flags:      0x0,
				},
				{
					Name:       "__typelink",
					Size:       0x550,
					Entropy:    3.713889346674307,
					VarEntropy: 0.00805212555333465,
					Flags:      0x0,
				},
				{
					Name:       "__itablink",
					Size:       0x78,
					Entropy:    2.662338733640253,
					VarEntropy: 0.03458699992230293,
					Flags:      0x0,
				},
				{
					Name:       "__gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "__gopclntab",
					Size:       0x610c0,
					Entropy:    5.469096410789927,
					VarEntropy: 2.2512068120373644e-05,
					Flags:      0x0,
				},
				{
					Name:       "__go_buildinfo",
					Size:       0x20,
					Entropy:    3.7959585933443494,
					VarEntropy: 0.052778240182421145,
					Flags:      0x0,
				},
				{
					Name:       "__nl_symbol_ptr",
					Size:       0x158,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x6,
				},
				{
					Name:       "__noptrdata",
					Size:       0x10780,
					Entropy:    5.599716173505525,
					VarEntropy: 0.00016908038331659258,
					Flags:      0x0,
				},
				{
					Name:       "__data",
					Size:       0x7470,
					Entropy:    1.7437891316045526,
					VarEntropy: 0.0003620706040999194,
					Flags:      0x0,
				},
				{
					Name:       "__bss",
					Size:       0x2f068,
					Entropy:    6.14028407220901,
					VarEntropy: 3.028528987721942e-05,
					Flags:      0x1,
				},
				{
					Name:       "__noptrbss",
					Size:       0x51c0,
					Entropy:    5.654580339454989,
					VarEntropy: 0.00037969970703756467,
					Flags:      0x1,
				},
				{
					Name:       "__zdebug_abbrev",
					Size:       0x117,
					Entropy:    7.166065824433167,
					VarEntropy: 0.0022405772861620005,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_line",
					Size:       0x1d5d0,
					Entropy:    7.990324720568736,
					VarEntropy: 2.1962513113172782e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_frame",
					Size:       0x5b90,
					Entropy:    7.931221335818104,
					VarEntropy: 8.399860586863567e-06,
					Flags:      0x2000000,
				},
				{
					Name:       "__debug_gdb_scri",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_info",
					Size:       0x33b99,
					Entropy:    7.996080385655248,
					VarEntropy: 5.2377625165921417e-08,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_loc",
					Size:       0x1a6ba,
					Entropy:    7.984926337700013,
					VarEntropy: 4.1118774999903916e-07,
					Flags:      0x2000000,
				},
				{
					Name:       "__zdebug_ranges",
					Size:       0x838f,
					Entropy:    7.891479517426951,
					VarEntropy: 8.821148933818937e-06,
					Flags:      0x2000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.1726267906855545,
					VarEntropy: 1.0666500878507862e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x358a0,
					Entropy:    4.348062560038336,
					VarEntropy: 5.6842742397058924e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x94,
					Entropy:    4.278922006970282,
					VarEntropy: 0.006074968357746798,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7941638566785665,
					VarEntropy: 0.008160113381189311,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.135389005131675,
					VarEntropy: 0.04770326580473294,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x593c8,
					Entropy:    5.45018051602998,
					VarEntropy: 2.5531099081952197e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.4681390622295662,
					VarEntropy: 0.08313705052107564,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.608059700687032,
					VarEntropy: 0.00016935717275402414,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x77f0,
					Entropy:    1.5859465586567814,
					VarEntropy: 0.00036012361195304527,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x3,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "linux",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       ".text",
					Size:       0x7fff6,
					Entropy:    6.171960423156391,
					VarEntropy: 1.0670534574051352e-05,
					Flags:      0x6,
				},
				{
					Name:       ".rodata",
					Size:       0x35940,
					Entropy:    4.355549400734324,
					VarEntropy: 5.691380954439535e-05,
					Flags:      0x2,
				},
				{
					Name:       ".shstrtab",
					Size:       0x17a,
					Entropy:    4.332514286812163,
					VarEntropy: 0.0018720932548837838,
					Flags:      0x0,
				},
				{
					Name:       ".typelink",
					Size:       0x4f0,
					Entropy:    3.7941638566785665,
					VarEntropy: 0.008160113381189311,
					Flags:      0x2,
				},
				{
					Name:       ".itablink",
					Size:       0x60,
					Entropy:    2.149135857994784,
					VarEntropy: 0.046165397750464555,
					Flags:      0x2,
				},
				{
					Name:       ".gosymtab",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x2,
				},
				{
					Name:       ".gopclntab",
					Size:       0x5a1e8,
					Entropy:    5.477551319776382,
					VarEntropy: 2.5029844485770247e-05,
					Flags:      0x2,
				},
				{
					Name:       ".go.buildinfo",
					Size:       0x20,
					Entropy:    3.5608203810934285,
					VarEntropy: 0.07164523745026193,
					Flags:      0x3,
				},
				{
					Name:       ".noptrdata",
					Size:       0x10720,
					Entropy:    5.608490555242617,
					VarEntropy: 0.00016935185165468147,
					Flags:      0x3,
				},
				{
					Name:       ".data",
					Size:       0x7810,
					Entropy:    1.6063005035044693,
					VarEntropy: 0.0003638549350251033,
					Flags:      0x3,
				},
				{
					Name:       ".bss",
					Size:       0x2ef48,
					Entropy:    7.993677019618252,
					VarEntropy: 9.19223826609501e-08,
					Flags:      0x3,
				},
				{
					Name:       ".noptrbss",
					Size:       0x5360,
					Entropy:    7.978067851913609,
					VarEntropy: 2.9303645810377574e-06,
					Flags:      0x3,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x119,
					Entropy:    7.186678878967755,
					VarEntropy: 0.0023487100483950583,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1b8a4,
					Entropy:    7.991902291817906,
					VarEntropy: 1.9590073032651564e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5526,
					Entropy:    7.922440004034238,
					VarEntropy: 1.016144764855772e-05,
					Flags:      0x0,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x2c,
					Entropy:    4.265583322887733,
					VarEntropy: 0.01731684668465522,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x31a81,
					Entropy:    7.995598638583296,
					VarEntropy: 6.238915579761367e-08,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x198ca,
					Entropy:    7.98781739055232,
					VarEntropy: 3.42170689161735e-07,
					Flags:      0x0,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x8fac,
					Entropy:    7.784901524718818,
					VarEntropy: 1.6544706982903474e-05,
					Flags:      0x0,
				},
				{
					Name:       ".note.go.buildid",
					Size:       0x64,
					Entropy:    5.393269689515109,
					VarEntropy: 0.011011736120437732,
					Flags:      0x2,
				},
				{
					Name:       ".symtab",
					Size:       0xc5e8,
					Entropy:    3.2070627267031826,
					VarEntropy: 0.0002941954027828894,
					Flags:      0x0,
				},
				{
					Name:       ".strtab",
					Size:       0xb288,
					Entropy:    4.811184615055419,
					VarEntropy: 4.984016326397377e-05,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x10fac8,
					Entropy:    5.8674506725463305,
					VarEntropy: 7.175418049736757e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640591917215783,
					VarEntropy: 0.0001537576469532067,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "plan9",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       "text",
					Size:       0x1108e8,
					Entropy:    5.877300605971006,
					VarEntropy: 7.1237047378367735e-06,
					Flags:      0x0,
				},
				{
					Name:       "data",
					Size:       0x17000,
					Entropy:    4.640976543009014,
					VarEntropy: 0.00015377671681234716,
					Flags:      0x0,
				},
				{
					Name:       "syms",
					Size:       0xe9ac,
					Entropy:    5.094666828676649,
					VarEntropy: 8.570249690578983e-05,
					Flags:      0x0,
				},
				{
					Name:       "spsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
				{
					Name:       "pcsz",
					Size:       0x0,
					Entropy:    0.0,
					VarEntropy: 0.0,
					Flags:      0x0,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "garble",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.174266111074433,
					VarEntropy: 9.747350147851416e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9d600,
					Entropy:    5.1315119143901295,
					VarEntropy: 1.6715070889304414e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.605522638411958,
					VarEntropy: 0.00015139773633553484,
					Flags:      0xc0000040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.5545243710280534,
					VarEntropy: 0.0051654859306460325,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.437586611009135,
					VarEntropy: 2.3924692928084093e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x200,
					Entropy:    0.020393135236084953,
					VarEntropy: 0.0003081937203618641,
					Flags:      0x42000000,
				},
			},
		},
		{
			name:    "SectionStats",
			goos:    "windows",
			builder: "go",
			flags:   "",
		}: sectionTestResults{
			sections: []Section{
				{
					Name:       ".text",
					Size:       0x8e400,
					Entropy:    6.176377421682298,
					VarEntropy: 9.748059471149685e-06,
					Flags:      0x60000020,
				},
				{
					Name:       ".rdata",
					Size:       0x9e600,
					Entropy:    5.140939553515747,
					VarEntropy: 1.648595339025535e-05,
					Flags:      0x40000040,
				},
				{
					Name:       ".data",
					Size:       0x17a00,
					Entropy:    4.601328355315124,
					VarEntropy: 0.0001514044694401641,
					Flags:      0xc0000040,
				},
				{
					Name:       ".zdebug_abbrev",
					Size:       0x200,
					Entropy:    4.829215920067953,
					VarEntropy: 0.024587620899965145,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_line",
					Size:       0x1cc00,
					Entropy:    7.993024884883035,
					VarEntropy: 1.6525580749442324e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_frame",
					Size:       0x5800,
					Entropy:    7.9257471578709,
					VarEntropy: 9.304797839881427e-06,
					Flags:      0x42100040,
				},
				{
					Name:       ".debug_gdb_scripts",
					Size:       0x200,
					Entropy:    0.7691902074556789,
					VarEntropy: 0.008919886275183865,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_info",
					Size:       0x32a00,
					Entropy:    7.996176726643122,
					VarEntropy: 5.3461931519680065e-08,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_loc",
					Size:       0x1ba00,
					Entropy:    7.988932433866987,
					VarEntropy: 2.8483765422067984e-07,
					Flags:      0x42100040,
				},
				{
					Name:       ".zdebug_ranges",
					Size:       0x9600,
					Entropy:    7.777719231416577,
					VarEntropy: 1.6851116730811515e-05,
					Flags:      0x42100040,
				},
				{
					Name:       ".idata",
					Size:       0x600,
					Entropy:    3.6148445724061844,
					VarEntropy: 0.005141851830632267,
					Flags:      0xc0000040,
				},
				{
					Name:       ".reloc",
					Size:       0x6a00,
					Entropy:    5.438815992577784,
					VarEntropy: 2.3712057587464778e-05,
					Flags:      0x42000040,
				},
				{
					Name:       ".symtab",
					Size:       0x17800,
					Entropy:    5.133872045459199,
					VarEntropy: 6.74350759437562e-05,
					Flags:      0x42000000,
				},
			},
		},
	},
}
